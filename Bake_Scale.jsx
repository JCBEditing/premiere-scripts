            ...
            for (var clipIndex = 0; clipIndex < clips.numItems; clipIndex++) {
                ...
                if (scaleh !== 100.00 || scalew !== 100.00 || effects[1].properties[1].getKeys() || effects[1].properties[2].getKeys){
                    ...
                    if (effects[1].properties[1].getKeys()){
                        numofclips++
                        var transkeys = effects[1].properties[1].getKeys();
                        var keytimes = []
                        var keyvalues = []
                        var keysection = []
                        var section = 0
                        var prevkeyvalue = [100, 100]
                        var keyvalue = [100, 100]
                        var index = 0
                        var currentkeytimes = []
                        

                        for (var i = 0; i < transkeys.length; i++) {
                            keyvalue = effects[1].properties[1].getValueAtKey(transkeys[i].seconds)
                            ...
                            // values come back as an array [100, 100]
                            ...
                            prevkeyvalue = effects[1].properties[1].getValueAtKey(transkeys[i].seconds)
                            index++
                        }
                        ...
                        for (var si = 0; si< keysection.length; si++){
                            ...
                            for (var vi = 0; vi < bakekeys; vi++) {
                                keyvalues[vi] = effects[1].properties[1].getValueAtTime(startTime + (iterate*vi));
                            }
                            // now we go back through and set the key and value
                            for (var vi = 0; vi < bakekeys; vi++) {
                                effects[1].properties[1].addKey(startTime + (iterate*vi));
                                effects[1].properties[1].setValueAtKey(startTime + (iterate*vi), keyvalues[vi] , 0)
                            }
                            amountofbakes++
                        }
                    }
                    //premiere uses a fraction of the screen to calculate translate 100 is the center if the anchor is also 100
                }              
            }	            
            app.setSDKEventMessage(amountofbakes + " Scale sections baked on " + numofclips + " clips.", 'info');
		} else {
